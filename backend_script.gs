...
function handleProcess1(data) {
  ...
  if (!sheet) {
    ...
    sheet.appendRow(['Timestamp', 'Collected By', 'Candidate Name', 'Passport Number', 'Address', 'Mobile', 'Email', 'Channel', 'Documents', 'Recruiter', 'Project', 'Remarks']);
  }
  
  sheet.appendRow([
    data.timestamp || new Date().toISOString(), data.collectedBy, data.candidateName, data.passportNumber,
    data.address, data.mobile, data.email, data.channel, data.documents, data.recruiter, data.project, data.remarks
  ]);
...
function handleProcess2(data) {
  ...
  if (!sheet) {
    ...
    sheet.appendRow(['Timestamp', 'Processed By', 'Candidate Name', 'Passport Number', 'Project', 'Documents Sent', 'Process Type', 'Agency Name', 'Agency Address', 'Send Type', 'Remarks']);
  }
  
  sheet.appendRow([
    data.timestamp || new Date().toISOString(), data.processedBy, data.candidateName, data.passportNumber,
    data.project, data.documentsSent, data.processType, data.agencyName, data.agencyAddress, data.sendType, data.remarks
  ]);
  return { status: 'success' };
}
...
function handleProcess4(data) {
  ...
  if (!sheet) {
    ...
    sheet.appendRow(['Timestamp', 'Received By', 'Candidate Name', 'Passport Number', 'Project', 'Documents Received', 'Process Type', 'Remarks']);
  }
  
  sheet.appendRow([
    data.timestamp || new Date().toISOString(), data.receivedBy, data.candidateName, data.passportNumber,
    data.project, data.documentsReceived, data.processType, data.remarks
  ]);
  return { status: 'success' };
}
...
function handleProcess3(data) {
  ...
  if (!sheet) {
    ...
    sheet.appendRow(['Timestamp', 'Returned By', 'Candidate Name', 'Passport Number', 'Project', 'Documents Returned', 'Reason', 'Send Type', 'Email', 'Mobile', 'Remarks']);
  }
  
  sheet.appendRow([
    data.timestamp || new Date().toISOString(), data.returnedBy, data.candidateName, data.passportNumber,
    data.project, data.documentsReturned, data.reason, data.sendType, data.email, data.mobile, data.remarks
  ]);
