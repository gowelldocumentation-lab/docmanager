...
async function fetchDtData(silent = false) {
  if (APPS_SCRIPT_URL === 'YOUR_WEB_APP_URL_HERE') {
      ...
      dtData.allData = {
        process1: [
          { 'Timestamp': new Date().toISOString(), 'Candidate Name': 'Test User', 'Passport Number': 'A1234567', 'Project': 'Project Alpha', 'Documents': 'SSLC, Plus Two, Degree', 'Collected By': 'Admin', 'File Number': 'GD/0101/26' },
          { 'Timestamp': new Date().toISOString(), 'Candidate Name': 'Jane Smith', 'Passport Number': 'B7654321', 'Project': 'Project Beta', 'Documents': 'Diploma, ITI', 'Collected By': 'Admin', 'File Number': 'GD/0102/26' }
        ],
        process2: [
          { 'Timestamp': new Date().toISOString(), 'Candidate Name': 'Test User', 'Passport Number': 'A1234567', 'Project': 'Project Alpha', 'Documents Sent': 'SSLC', 'Process Type': 'Visa Stamping', 'Processed By': 'Admin', 'File Number': 'GD/0101/26' }
        ],
        process3: [],
        process4: []
      };
...
function populateCandidateDropdowns() {
  ...
  if (repCand) {
    ...
    uniqueCands.sort((a,b) => a.name.localeCompare(b.name)).forEach(c => {
      repCand.innerHTML += `<option value="${c.passport}">${c.name} - ${c.passport} (${c.fileNumber || 'No File #'})</option>`;
    });
  }
}
...
window.handleProjectDropdownChange = function(prefix) {
  ...
  candidatesRegistry.forEach((c, index) => {
    if (c.project === proj) {
      select.innerHTML += `<option value="${index}">${c.name} - ${c.passport} (${c.fileNumber || 'No File #'})</option>`;
    }
  });
}
...
async function handleP1ChannelChange(selectElement) {
  await handleDropdownAddOption(selectElement, 'channel');
  const val = selectElement.value || '';
  const templateGroup = document.getElementById('dt-p1-template-group');
  if (val.includes('Direct')) {
    templateGroup.style.display = 'flex';
    templateGroup.style.flexDirection = 'column';
  } else {
...
function selectCandidate(prefix, candidate) {
  ...
  card.innerHTML = `
    <div style="display:flex; justify-content:space-between; align-items:flex-start;">
      <div>
        <strong>${candidate.name}</strong> (${candidate.passport}) - <span style="background:var(--bg-card-hover); padding: 2px 6px; border-radius: 4px; font-size: 0.85rem; border: 1px solid var(--border-color);">${candidate.fileNumber || 'No File #'}</span><br>
...
async function submitDtProcess1(e) {
  ...
  const payload = {
    ...
    position: document.getElementById('dt-p1-position').value,
    templateType: document.getElementById('dt-p1-template-group').style.display !== 'none' ? document.getElementById('dt-p1-template').value : "",
    remarks: document.getElementById('dt-p1-remarks').value
  };
  
  let maxSeq = 100;
  if (dtData.allData && dtData.allData.process1) {
    dtData.allData.process1.forEach(row => {
      const fn = row['File Number'];
      if (fn && fn.startsWith('GD/')) {
  ...
  payload.fileNumber = `GD/${nextSeq}/${currentYear}`;
  
  let loadingText = null;
  if (payload.templateType === 'Technical' || payload.templateType === 'Healthcare') {
    loadingText = '<i class="fa-solid fa-circle-notch fa-spin"></i> Submitting & Generating PDF...';
  }

  const response = await submitDtData(payload, "Collection form submitted.", btn, loadingText);
  if(response) {
    if (response.pdfUrl) {
      ...
      alert("Warning: PDF could not be generated. Error from server: " + (response.pdfError || "Unknown error (likely permissions)."));
    } else if (!payload.templateType && payload.channel.includes('Direct')) {
    ...
    dtData.allData.process1.push({
      ...
      'Channel': payload.channel, 'Position': payload.position, 'Template': payload.templateType, 'Documents': payload.documents, 'Recruiter': payload.recruiter, 'Project': payload.project, 'Remarks': payload.remarks, 'File Number': payload.fileNumber
    });
    buildCandidateRegistry();
...
async function submitDtProcess2(e) {
  ...
  if(success) { 
    dtData.allData.process2.push({
      ...
      'Send Type': payload.sendType, 'Remarks': payload.remarks, 'File Number': payload.fileNumber
    });
    buildCandidateRegistry();
...
async function submitDtProcess4(e) {
  ...
  if(success) { 
    dtData.allData.process4.push({
      ...
      'Process Type': payload.processType, 'Remarks': payload.remarks, 'File Number': payload.fileNumber
    });
    buildCandidateRegistry();
...
async function submitDtProcess3(e) {
  ...
  if(success) {
    ...
    dtData.allData.process3.push({
      ...
      'Reason': payload.reason, 'Send Type': payload.sendType, 'Email': payload.email, 'Mobile': payload.mobile, 'Remarks': payload.remarks, 'File Number': payload.fileNumber
    });
    buildCandidateRegistry();
...
async function submitDtData(payload, successMsg, submitBtn = null, customLoadingText = null) {
  if (APPS_SCRIPT_URL === 'YOUR_WEB_APP_URL_HERE') {
  ...
  if (submitBtn) {
    ...
    submitBtn.innerHTML = customLoadingText || '<i class="fa-solid fa-circle-notch fa-spin"></i> Submitting...';
...
function printAcknowledgment(type, payload) {
  ...
  if (type === 'collect' && (payload.templateType === 'Technical' || payload.templateType === 'Healthcare')) {
    htmlContent = `
      <html>
        <head>
          <title>Document Received Template for ${payload.templateType}</title>
          <style>
            body { font-family: 'Times New Roman', serif; padding: 20px 40px; color: #000; line-height: 1.5; }
            .header-text { text-align: center; margin-bottom: 20px; border-bottom: 1px solid #000; padding-bottom: 10px; }
            .header-title { font-size: 18px; font-weight: bold; text-transform: uppercase; margin: 0; }
            .header-subtitle { font-size: 12px; margin: 5px 0; }
            .doc-title { text-align: center; font-size: 16px; font-weight: bold; margin: 20px 0; text-decoration: underline; text-transform: uppercase; }
            ...
            .declaration { margin-top: 20px; text-align: justify; }
        ...
        <body>
          <div class="header-text">
            <h1 class="header-title">Gowell International Recruitment Consultancy</h1>
            <p class="header-subtitle">(B-2398/KER/PER/1000+/5/9380/2018) Approved by the Ministry of External Affairs, Govt. Of India<br>
            XX-855/3, 2nd Floor, CRS Tower, Govt. Hospital Junction, Aluva, Ernakulam, Kerala, India. Pin – 683101.<br>
            Phone: +91- 484 2633358 | +91 9778590527 | +91 74797 47936</p>
          </div>
          <div class="doc-title">DOCUMENT RECEIVED TEMPLATE FOR ${payload.templateType.toUpperCase()}</div>
          <div class="content">
            <div class="field-row"><div class="field-label">Date & Time:</div><div class="field-value">${new Date(payload.timestamp || Date.now()).toLocaleString()}</div></div>
            <div class="field-row"><div class="field-label">Candidate Name:</div><div class="field-value">${payload.candidateName}</div></div>
            <div class="field-row"><div class="field-label">Passport Number:</div><div class="field-value">${payload.passportNumber}</div></div>
            <div class="field-row"><div class="field-label">File Number:</div><div class="field-value">${payload.fileNumber}</div></div>
            <div class="field-row"><div class="field-label">Position/Category:</div><div class="field-value">${payload.position || ''}</div></div>
            <div class="field-row"><div class="field-label">Project:</div><div class="field-value">${payload.project}</div></div>
            <div class="field-row"><div class="field-label">Documents Received:</div><div class="field-value">${payload.documents}</div></div>
            <div class="field-row"><div class="field-label">Mobile Number:</div><div class="field-value">${payload.mobile}</div></div>
            <div class="field-row"><div class="field-label">Email ID:</div><div class="field-value">${payload.email}</div></div>
            
            <div class="declaration">
              <p>I hereby declare that I have submitted the above-mentioned documents to Gowell International Recruitment Consultancy for the purpose of processing my application for the position of <strong>${payload.position || '___________'}</strong> under the <strong>${payload.project}</strong> project.</p>
              <p>I understand that these documents will be used strictly for recruitment purposes and will be handled in accordance with the company's privacy policy.</p>
            </div>
            ...
            <div class="declaration">
              <ol>
                <li>The candidate must provide authentic and original documents when required.</li>
                <li>Gowell International is not responsible for any delay caused by external verification agencies.</li>
                <li>The candidate agrees to attend all scheduled interviews and medical examinations.</li>
                <li>Any falsification of documents will result in immediate termination of the application process.</li>
                <li>This receipt must be produced when collecting the documents back.</li>
              </ol>
            </div>
          ...
          <div class="signatures">
            <div class="sig-box">
              <div class="sig-line">Signature of the Candidate<br><br>${payload.candidateName}</div>
            </div>
            <div class="sig-box">
              <div class="sig-line">For Gowell International Recruitment Consultancy<br><br>Authorized Signatory (${payload.collectedBy || loggedInUser})</div>
            </div>
          </div>
        </body>
      </html>
    ...
    if (type === 'collect') {
      ...
      contentHtml = `
        ...
        <div class="field"><span class="label">File Number:</span> ${payload.fileNumber}</div>
    ...
    htmlContent = `
      <html>
        <head>
          <title>${title}</title>
          <style>
            body { font-family: Arial, sans-serif; padding: 40px; color: #333; line-height: 1.6; position: relative; }
            .watermark {
              position: fixed;
              top: 50%;
              left: 50%;
              ...
              width: 80%;
              height: 80%;
              background-image: url('https://res.cloudinary.com/dpnb1to3s/image/upload/v1781796016/481778660_1037742571707490_6757743138098457662_n-removebg-preview_omsd29.png');
            ...
            .letterhead-header {
              display: flex;
              align-items: center;
              border-bottom: 2px solid #1a4a7b;
              padding-bottom: 15px;
              margin-bottom: 30px;
            }
            .lh-logo {
        ...
        <body>
          ...
          <div class="letterhead-header">
            <img src="https://res.cloudinary.com/dpnb1to3s/image/upload/v1781796016/481778660_1037742571707490_6757743138098457662_n-removebg-preview_omsd29.png" alt="Logo" class="lh-logo">
            <div class="lh-details">
              <h1 class="lh-title">Gowell International Recruitment Consultancy</h1>
              <p class="lh-address">2nd Floor, C.R.S Tower, XX-855/3, Parakkal Cir, behind SBI Bank, Kunnathery Thikavu, Aluva, Kerala 683101</p>
              <p class="lh-contact">Phone: 074797 47936</p>
            </div>
          </div>
...
function renderReports() {
  ...
  let allOps = [];
  if (dtData.allData) {
    dtData.allData.process1.forEach(r => allOps.push({...r, _cat: 'Collection', _op: 'Collected', _by: r['Collected By'], _docs: r['Documents'], _fileNo: r['File Number']}));
    dtData.allData.process2.forEach(r => allOps.push({...r, _cat: 'Processing', _op: `Sent (${r['Process Type']})`, _by: r['Processed By'], _docs: r['Documents Sent'], _fileNo: r['File Number']}));
    dtData.allData.process4.forEach(r => allOps.push({...r, _cat: 'Received', _op: 'Received Back', _by: r['Received By'], _docs: r['Documents Received'], _fileNo: r['File Number']}));
    dtData.allData.process3.forEach(r => allOps.push({...r, _cat: 'Return', _op: `Returned (${r['Reason']})`, _by: r['Returned By'], _docs: r['Documents Returned'], _fileNo: r['File Number']}));
  }
...
function viewReportDetails(index) {
  ...
  // Define fields we want to skip initially so we can reorder or handle them specifically
  const skipFields = ['_cat', '_op', '_by', '_docs', '_fileNo', '_isMissing'];
  
  // Include ALL original fields that exist in the row.
  ...
  if (row._cat) {
    html += `
        <tr style="border-bottom: 1px solid var(--border-color);">
          <td style="padding: 8px 0; font-weight:600; width:40%; color:var(--text-muted);">Operation Category</td>
          <td style="padding: 8px 0;">${row._cat}</td>
        </tr>
    `;
  }
  if (row._op) {
    html += `
        <tr style="border-bottom: 1px solid var(--border-color);">
          <td style="padding: 8px 0; font-weight:600; width:40%; color:var(--text-muted);">Operation Status</td>
          <td style="padding: 8px 0;">${row._op}</td>
        </tr>
    `;
  }
  if (row._by) {
    html += `
        <tr style="border-bottom: 1px solid var(--border-color);">
          <td style="padding: 8px 0; font-weight:600; width:40%; color:var(--text-muted);">Action Performed By</td>
          <td style="padding: 8px 0; font-weight:bold; color:var(--primary-color);">${row._by}</td>
        </tr>
    `;
  }
  ...
  // Add Print PDF button for Direct / Direct Office
  const channel = row['Channel'] || row['Send Type'];
  if (channel === 'Direct' || channel === 'Direct Office') {
     let type = row._cat === 'Collection' ? 'collect' : (row._cat === 'Return' ? 'return' : null);
     if (type) {
         let printPayload = {
             timestamp: row.Timestamp,
             candidateName: row['Candidate Name'] || '',
             passportNumber: row['Passport Number'] || '',
             fileNumber: row['File Number'] || row._fileNo || '',
             project: row['Project'] || '',
             documents: row['Documents'] || row['Documents Sent'] || row['Documents Received'] || row['Documents Returned'] || '',
             documentsReturned: row['Documents Returned'] || '',
             reason: row['Reason'] || '',
             remarks: row['Remarks'] || ''
         };
         
         const payloadStr = JSON.stringify(printPayload).replace(/'/g, "&#39;").replace(/"/g, "&quot;");
         
         html += `<div style="margin-top:20px; text-align:center;">
                   <button class="btn-primary" onclick="printAcknowledgment('${type}', JSON.parse('${payloadStr}'))">
                     <i class="fa-solid fa-file-pdf"></i> Print PDF Document
                   </button>
                 </div>`;
     }
  }
