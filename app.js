...
async function submitDtProcess1(e) {
  ...
  if(success) {
    ...
    dtData.allData.process1.push({
      ...
      'Channel': payload.channel, 'Documents': payload.documents, 'Recruiter': payload.recruiter, 'Project': payload.project, 'Remarks': payload.remarks, 'File Number': payload.fileNumber
    });
    buildCandidateRegistry();
...
function viewReportDetails(index) {
  ...
  // Also explicitly add operation details for clarity
  if (row._cat) {
    html += `
        <tr style="border-bottom: 1px solid var(--border-color);">
